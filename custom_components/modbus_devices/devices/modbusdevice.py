...
class ModbusDevice:
    ...
    def __init__(self, connection_params: ConnectionParams, rtu_bus: RTUBusManager):
        if isinstance(connection_params, TCPConnectionParams):
            self._client = AsyncModbusTcpClient(
                host=connection_params.ip, port=connection_params.port
            )
    ...
    def close(self):
        ...
        try:
            _LOGGER.debug(
                "Shutting down and disconnecting device: %s %s",
                self.manufacturer,
                self.model,
            )
            self._client.close()
            ...
            _LOGGER.warning(
                "Error closing client for device %s %s: %s",
                self.manufacturer,
                self.model,
                e,
            )
    ...
    def loadConfigUI(self):
        ...
        if not config_group:
            return  # Nothing to do if CONFIG is empty

        # Check if CONFIG group has any number or select datapoints
        has_number = any(
            isinstance(dp.entity_data, EntityDataNumber) for dp in config_group.values()
        )
        has_select = any(
            isinstance(dp.entity_data, EntityDataSelect) for dp in config_group.values()
        )

        if not (has_number or has_select):
            return  # No relevant datapoints, nothing to add

        # Add the selector and any required value displays
        ui_group["Config Selection"] = ModbusDatapoint(
            entity_data=EntityDataSelect(category=EntityCategory.CONFIG)
        )
        ui_group.update(
            {
                "Config Value Select": ModbusDatapoint(
                    entity_data=EntityDataSelect(category=EntityCategory.CONFIG)
                )
            }
            if has_select
        ...
        ui_group.update(
            {
                "Config Value Number": ModbusDatapoint(
                    entity_data=EntityDataNumber(category=EntityCategory.CONFIG)
                )
            }
            if has_number
            else {}
    ...
    async def readGroup(self, group: ModbusGroup):
        ...
        addresses = [
            (dp.address, dp.register_count) for dp in self.Datapoints[group].values()
        ]
        if not addresses:
            return
        ...
        response = await method(
            address=start_addr, count=n_reg, device_id=self._slave_id
        )
        ...
        data = (
            response.bits
            if group.mode in (ModbusMode.COILS, ModbusMode.DISCRETE_INPUTS)
        ...
        for name, dp in self.Datapoints[group].items():
            ...
            try:
                ...
                _LOGGER.warning(
                    "Failed to decode datapoint %s in group %s (addr=%s len=%s raw=%s)",
                    name,
                    group,
                    dp.address,
                    dp.register_count,
    ...
    async def readValue(self, group: ModbusGroup, key: str) -> float | str:
        ...
        method = self._get_read_method(group.mode)
        response = await method(
            address=dp.address, count=register_count, device_id=self._slave_id
        )
    ...
    async def writeValue(self, group: ModbusGroup, key: str, value: float):
        ...
        if register_count > 2:
            raise ValueError(
                f"Unsupported register count: {register_count}. Only 1 or 2 registers are supported."
            )
        ...
        if group.mode == ModbusMode.COILS:
            method = (
                ...
                if register_count == 1
                else self._client.write_coils
            )
        elif group.mode == ModbusMode.HOLDING:
            method = (
                self._client.write_register
                if register_count == 1
                else self._client.write_registers
            )
        else:
            raise ModbusException(
                f"Write Value: Unsupported Modbus mode {group.mode!r} for group {group!r}"
            )
