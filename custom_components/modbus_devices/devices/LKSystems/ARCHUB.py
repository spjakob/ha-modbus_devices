...
class Device(ModbusDevice):
    ...
    # Define static groups (dynamic groups are created in onAfterFirstRead)
...
#    GROUP_SETPOINTS = ModbusGroup(ModbusMode.HOLDING, ModbusPollMode.POLL_ON)
    ...
    def onAfterFirstRead(self):
        # Update device info
        serial_registers = self.Datapoints[self.GROUP_DEVICE_INFO]["Serial Number"].Value
...
#        self.serial_number = struct.unpack('>Q', packed_bytes)[0]
        ...
        for i in range(1, self.number_of_zones + 1):
            # Create a dynamic group name and object for each zone
            dynamic_group_name = f"GROUP_SENSORS_ZONE_{i}"
            dynamic_group = ModbusGroup(ModbusMode.INPUT, ModbusPollMode.POLL_ON)
        ...
        for i in range(1, self.number_of_zones + 1):
            # Create a dynamic group name and object for each zone
            dynamic_group_name = f"GROUP_SETPOINTS_ZONE_{i}"
            dynamic_group = ModbusGroup(ModbusMode.HOLDING, ModbusPollMode.POLL_ON)
