...
class Device(ModbusDevice):
    ...
    def loadDatapoints(self):
        ...
        self.Datapoints[GROUP_DEVICE_INFO] = {
            "Serial Number": ModbusDatapoint(
                address=0, register_count=4, type="uint"
            ),  # 4 registers for a 64-bit value
        ...
        self.Datapoints[GROUP_UNIT_STATUSES] = {
            "Actuator 1": ModbusDatapoint(
                address=60,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 2": ModbusDatapoint(
                address=61,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 3": ModbusDatapoint(
                address=62,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 4": ModbusDatapoint(
                address=63,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 5": ModbusDatapoint(
                address=64,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 6": ModbusDatapoint(
                address=65,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 7": ModbusDatapoint(
                address=66,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 8": ModbusDatapoint(
                address=67,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 9": ModbusDatapoint(
                address=68,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 10": ModbusDatapoint(
                address=69,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 11": ModbusDatapoint(
                address=70,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
            ),
            "Actuator 12": ModbusDatapoint(
                address=71,
                entity_data=EntityDataSensor(
                    enum={0: "Closed", 1: "Open", 2: "Unallocated"}
                ),
        ...
        self.Datapoints[GROUP_ALARMS] = {
            "Cooling Emergency Mode": ModbusDatapoint(
                address=80,
                entity_data=EntityDataSensor(
                    enum={0: "Normal Mode", 1: "Emergency Mode"}
                ),
            ),
        }
        ...
        self.Datapoints[GROUP_COMMANDS] = {
            "Operating Mode": ModbusDatapoint(
                ...
                entity_data=EntityDataSelect(
                    options={0: "Undefined", 1: "Heating", 2: "Cooling"}
                ),
            ),
            "LED Enable": ModbusDatapoint(
                address=58,
                entity_data=EntityDataSelect(options={0: "Disable", 1: "Enable"}),
            ),
        }

        # CONFIGURATION - Read/Write
        self.Datapoints[ModbusDefaultGroups.CONFIG] = {
            "Temperature Alarm High Level": ModbusDatapoint(
                address=50,
                scaling=0.1,
            ...
            "Temperature Alarm Low Level": ModbusDatapoint(
                address=51,
                scaling=0.1,
                entity_data=EntityDataNumber(
                    deviceClass=NumberDeviceClass.TEMPERATURE,
            ...
            "Humidity Alarm High Level": ModbusDatapoint(
                address=52,
                scaling=0.1,
                entity_data=EntityDataNumber(
            ...
            "Humidity Alarm Low Level": ModbusDatapoint(
                address=53,
                scaling=0.1,
                entity_data=EntityDataNumber(
                    deviceClass=NumberDeviceClass.HUMIDITY,
            ...
            "Battery Alarm Low Level": ModbusDatapoint(
                address=54,
                scaling=0.1,
                entity_data=EntityDataNumber(
                    deviceClass=NumberDeviceClass.BATTERY,
                    units=PERCENTAGE,
                    min_value=0,
                    max_value=100,
                    step=0.1,
            ...
            "Battery Alarm Critical Level": ModbusDatapoint(
                address=55,
                scaling=0.1,
                entity_data=EntityDataNumber(
                    deviceClass=NumberDeviceClass.BATTERY,
            ...
            "Cooling Emergency Number of Zones": ModbusDatapoint(
                address=56, entity_data=EntityDataNumber(min_value=0, max_value=12)
            ),
            "Coling Mode Humidity Limit": ModbusDatapoint(
                address=57,
                scaling=0.1,
                entity_data=EntityDataNumber(
                    deviceClass=NumberDeviceClass.HUMIDITY,
    ...
    def onAfterFirstRead(self):
        ...
        _LOGGER.info(
            "%s zones detected and activating entities for these", number_of_zones
        )

        # Dynamically assign SENSOR datapoints to a separate group for each zone
        for i in range(1, int(number_of_zones) + 1):
            # Create a new dynamic group
            self.dynamic_groups[f"GROUP_SENSORS_ZONE_{i}"] = ModbusGroup(
                ModbusMode.INPUT, ModbusPollMode.POLL_ON
            )

            base_register = i * 100
            _LOGGER.debug(
                "Setting up zone %s adding temperature register %s ", i, base_register
            )
            ...
            self.Datapoints[self.dynamic_groups[f"GROUP_SENSORS_ZONE_{i}"]] = {
                ...
                f"Zone {i} Connected Actuators": ModbusDatapoint(
                    address=base_register + 7, type="uint"
                ),  # Corrected from +6 to +7
            }

        # Dynamically assign SETPOINT datapoints to a separate group for each zone
        for i in range(1, number_of_zones + 1):
            # Create a new dynamic group
            self.dynamic_groups[f"GROUP_SETPOINTS_ZONE_{i}"] = ModbusGroup(
                ModbusMode.HOLDING, ModbusPollMode.POLL_ON
            )
        ...
        for i in range(1, int(number_of_zones) + 1):
            self.Datapoints.setdefault(GROUP_UNIT_STATUSES, {})[
                f"Zone {i} Thermostat Address"
            ] = ModbusDatapoint(
                entity_data=EntityDataSensor(icon="mdi:network-outline")
            )
            self.Datapoints.setdefault(GROUP_UNIT_STATUSES, {})[
                f"Zone {i} Connected Actuators List"
            ] = ModbusDatapoint(entity_data=EntityDataSensor(icon="mdi:valve"))
    ...
    def onAfterRead(self):
        ...
        for i in range(1, int(number_of_zones) + 1):
            ...
            try:
                raw_mac_int = self.Datapoints[
                    self.dynamic_groups[f"GROUP_SENSORS_ZONE_{i}"]
                ][f"Zone {i} Thermostat Address Raw"].value
            ...
            try:
                bitfield = self.Datapoints[
                    self.dynamic_groups[f"GROUP_SENSORS_ZONE_{i}"]
                ][f"Zone {i} Connected Actuators"].value
