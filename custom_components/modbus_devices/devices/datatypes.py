...
class EntityDataSensor(EntityData):
    ...
    stateClass: str = (
        None  # None | Set to valid "SensorStateClass" to enable long term storage
    )
...
class ModbusDatapoint:
    ...
    def from_modbus(
        ...
        if len(registers) != self.register_count:
            raise ValueError(
                f"Datapoint at address {self.address}: expected {self.register_count} registers, got {len(registers)}"
            )
        ...
        for reg in registers:
            b += reg.to_bytes(
                2, byteorder="big" if byte_order == ByteOrder.MSB else "little"
            )
        ...
        if self.type in (ModbusDataType.INT, ModbusDataType.UINT):
            combined_value = int.from_bytes(
                b, byteorder="big", signed=(self.type == ModbusDataType.INT)
            )
            ...
            try:
                ...
                self.value = b_chars.split(b"\x00", 1)[0].decode(
                    "ascii", errors="ignore"
                )
            except Exception:
                self.value = ""
    ...
    def to_modbus(
        ...
        # Apply word swap if needed
        if (
            word_order == WordOrder.SWAP
            and self.register_count > 1
            and self.type not in (ModbusDataType.STRING1, ModbusDataType.STRING2)
        ):
            b = self.modbus_word_swap(b)
        ...
        for i in range(0, len(b), 2):
            registers.append(
                int.from_bytes(
                    b[i : i + 2],
                    byteorder="big" if byte_order == ByteOrder.MSB else "little",
                )
            )
